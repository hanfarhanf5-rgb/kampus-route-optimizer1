...
def hill_climbing():
    ...
    steps = [{"route": best[:], "distance": best_dist, "local_opt": False}]
    improved = True
    while improved:
        improved = False
    ...
    return steps, round(time.time()-start_time, 3)
...
def simulated_annealing():
    ...
    steps = [{"route": current[:], "distance": current_dist, "local_opt": False}]
    T = 1.0
    T_min = 0.001
    alpha = 0.9
    while T > T_min:
...
def genetic_algorithm(pop_size=6, generations=10):
    start_time = time.time()
...
def optimize():
    ...
    if algo == "hc":
        steps, time_exec = hill_climbing()
    elif algo == "sa":
        steps, time_exec = simulated_annealing()
    else:
        steps, time_exec = genetic_algorithm()
    return jsonify({"steps": steps, "time": time_exec, "algorithm": algo})
